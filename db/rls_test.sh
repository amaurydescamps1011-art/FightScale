...
insert into club (id, nom, ville, statut, offre) values
  ('aaaaaaaa-0000-0000-0000-000000000001', 'Club A', 'Marseille', 'en_attente', 'gratuit'),
  ('bbbbbbbb-0000-0000-0000-000000000002', 'Club B', 'Lyon',      'publie',     'pro'),
  ('cccccccc-0000-0000-0000-000000000003', 'Club C', 'Paris',     'en_attente', 'gratuit'),
  ('dddddddd-0000-0000-0000-000000000004', 'Club D', 'Nice',      'publie',     'gratuit');
...
ok "le visiteur ne voit que les clubs publies" \
   "$(q "set role anon; select coalesce(string_agg(nom,', ' order by nom),'(rien)') from club;")" "Club B, Club D "
ok "le gerant voit son club et les publies" \
   "$(q "$(cnx $A) select string_agg(nom,', ' order by nom) from club;")" "Club A, Club B, Club D "
